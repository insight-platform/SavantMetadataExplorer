...
export class FrameTreeComponent implements OnChanges {
  ...
  private _createSvg(treemapNodes: HierarchyNode<INodeObject>, treeWidth: number): void {
    ...
    const toggleColor = (event, d) => {
      if (!this.selectedNode) {
        ...
        currentColor = colorDeepMap[this.colorPalette];
        textColor = '#fff';
        ...
        d3.select('#text' + this.selectedNode.id).selectAll('tspan').style('fill', '#000');
      ...
      d3.select('#text' + d.data.id).selectAll('tspan').style('fill', textColor);
  ...
  private _getColor(data: INodeObject, alfa = 1, applyFilter = true): string {
    ...
    if (groupIndex === -1) {
      return getColor(-1, alfa, this.colorPalette);
    }

    return getColor((groupIndex + 1) / this.objectGroups.length, alfa, this.colorPalette);
  }
